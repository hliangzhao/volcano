...
func TestAdmitQueues(t *testing.T) {
	...
	stateNotSet := schedulingv1alpha1.Queue{
	...
	openState := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
		...
		Status: schedulingv1alpha1.QueueStatus{
			State: schedulingv1alpha1.QueueStateOpen,
		},
	}
	...
	closedState := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
		...
		Status: schedulingv1alpha1.QueueStatus{
			State: schedulingv1alpha1.QueueStateClosed,
		},
	}
	...
	wrongState := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "abnormal-case",
		},
		Spec: schedulingv1alpha1.QueueSpec{
	...
	openStateForDelete := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
	...
	closedStateForDelete := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
	...
	weightNotSet := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "weight-not-set",
		},
		Spec: schedulingv1alpha1.QueueSpec{},
	...
	negativeWeight := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
	...
	positiveWeightForUpdate := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
	...
	negativeWeightForUpdate := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "positive-weight-for-update",
		},
		Spec: schedulingv1alpha1.QueueSpec{
	...
	hierarchyWeightsDontMatch := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "hierarchy-weights-dont-match",
			Annotations: map[string]string{
				schedulingv1alpha1.KubeHierarchyAnnotationKey:       "root/a/b",
				schedulingv1alpha1.KubeHierarchyWeightAnnotationKey: "1/2/3/4",
			},
		},
		Spec: schedulingv1alpha1.QueueSpec{
			Weight: 1,
	...
	hierarchyWeightsNegative := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "hierarchy-weights-dont-match",
			Annotations: map[string]string{
				schedulingv1alpha1.KubeHierarchyAnnotationKey:       "root/a/b",
				schedulingv1alpha1.KubeHierarchyWeightAnnotationKey: "1/-1/3",
			},
		},
		Spec: schedulingv1alpha1.QueueSpec{
			Weight: 1,
	...
	weightsFormatIllegal := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "hierarchy-weights-dont-match",
			Annotations: map[string]string{
				schedulingv1alpha1.KubeHierarchyAnnotationKey:       "root/a/b",
				schedulingv1alpha1.KubeHierarchyWeightAnnotationKey: "1/a/3",
			},
		},
		Spec: schedulingv1alpha1.QueueSpec{
			Weight: 1,
	...
	ordinaryHierchicalQueue := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "ordinary-hierarchical-queue",
			Annotations: map[string]string{
				schedulingv1alpha1.KubeHierarchyAnnotationKey:       "root/node1/node2",
				schedulingv1alpha1.KubeHierarchyWeightAnnotationKey: "1/2/3",
			},
		},
		Spec: schedulingv1alpha1.QueueSpec{
			Weight: 1,
	...
	hierarchicalQueueInSubPathOfAnotherQueue := schedulingv1alpha1.Queue{
		ObjectMeta: metav1.ObjectMeta{
			Name: "hierarchical-queue-in-sub-path-of-another-queue",
			Annotations: map[string]string{
				schedulingv1alpha1.KubeHierarchyAnnotationKey:       "root/node1",
				schedulingv1alpha1.KubeHierarchyWeightAnnotationKey: "1/4",
			},
		},
		Spec: schedulingv1alpha1.QueueSpec{
			Weight: 1,
	...
	_, err = config.VolcanoClient.SchedulingV1alpha1().Queues().Create(context.TODO(), &openStateForDelete, metav1.CreateOptions{})
	if err != nil {
		t.Errorf("Create queue with open state failed for %v.", err)
	}

	_, err = config.VolcanoClient.SchedulingV1alpha1().Queues().Create(context.TODO(), &closedStateForDelete, metav1.CreateOptions{})
	if err != nil {
		t.Errorf("Create queue with closed state failed for %v.", err)
	}

	_, err = config.VolcanoClient.SchedulingV1alpha1().Queues().Create(context.TODO(), &ordinaryHierchicalQueue, metav1.CreateOptions{})
	if err != nil {
		t.Errorf("Create hierarchical queue failed for %v.", err)
	}
	_, err = config.VolcanoClient.SchedulingV1alpha1().Queues().Create(context.TODO(), &positiveWeightForUpdate, metav1.CreateOptions{})
	if err != nil {
		t.Errorf("Crate queue with positive weight failed for %v.", err)
	}

	defer func() {
		if err := config.VolcanoClient.SchedulingV1alpha1().Queues().Delete(context.TODO(), openStateForDelete.Name, metav1.DeleteOptions{}); err != nil {
			fmt.Printf("Delete queue with open state failed for %v.\n", err)
		}
		if err := config.VolcanoClient.SchedulingV1alpha1().Queues().Delete(context.TODO(), closedStateForDelete.Name, metav1.DeleteOptions{}); err != nil {
			fmt.Printf("Delete queue with closed state failed for %v.\n", err)
		}
		if err := config.VolcanoClient.SchedulingV1alpha1().Queues().Delete(context.TODO(), ordinaryHierchicalQueue.Name, metav1.DeleteOptions{}); err != nil {
	...
	testCases := []struct {
			...
			reviewResponse: &admissionv1.AdmissionResponse{
				...
				Result: &metav1.Status{
					Message: field.Invalid(field.NewPath("requestBody").Child("spec").Child("state"),
						"wrong", fmt.Sprintf("queue state must be in %v", []schedulingv1alpha1.QueueState{
		...
		},
		{
			...
			reviewResponse: utils.ToAdmissionResponse(fmt.Errorf("invalid operation `%s`, "+
