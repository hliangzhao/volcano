...
func validatePolicies(polices []batchv1alpha1.LifecyclePolicy, fieldPath *field.Path) error {
	var err error
	...
	for _, policy := range polices {
		if (policy.Event != "" || len(policy.Events) != 0) && policy.ExitCode != nil {
			err = gomultierror.Append(err, fmt.Errorf("must not specify event and exitCode simultaneously"))
			break
		}

		if policy.Event == "" && len(policy.Events) == 0 && policy.ExitCode == nil {
			err = gomultierror.Append(err, fmt.Errorf("either event and exitCode should be specified"))
			break
		}
		...
		if len(policy.Event) != 0 || len(policy.Events) != 0 {
			...
			for _, event := range policyEventsList {
				if allow, ok := policyEventMap[event]; !ok || !allow {
					err = gomultierror.Append(err, field.Invalid(fieldPath, event, "invalid policy event"))
					bFlag = true
					break
				}

				if allow, ok := policyActionMap[policy.Action]; !ok || !allow {
					err = gomultierror.Append(err, field.Invalid(fieldPath, policy.Action, "invalid policy action"))
					bFlag = true
					break
				}
				if _, found := policyEvents[event]; found {
					err = gomultierror.Append(err, fmt.Errorf("duplicate event %v  across different policy", event))
					bFlag = true
					break
				} else {
			...
			if *policy.ExitCode == 0 {
				err = gomultierror.Append(err, fmt.Errorf("0 is not a valid error code"))
				break
			}
			if _, found := exitCodes[*policy.ExitCode]; found {
				err = gomultierror.Append(err, fmt.Errorf("duplicate exitCode %v", *policy.ExitCode))
				break
			} else {
	...
	if _, found := policyEvents[busv1alpha1.AnyEvent]; found && len(policyEvents) > 1 {
		err = gomultierror.Append(err, fmt.Errorf("if there's * here, no other policy should be here"))
	}
