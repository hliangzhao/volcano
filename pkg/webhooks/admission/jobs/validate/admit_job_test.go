...
func TestValidateJobCreate(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			defaultqueue := schedulingv1alpha1.Queue{
				ObjectMeta: metav1.ObjectMeta{
					Name: "default",
				},
			...
			_, err := config.VolcanoClient.SchedulingV1alpha1().Queues().Create(context.TODO(), &defaultqueue, metav1.CreateOptions{})
			if err != nil {
...
func TestValidateJobUpdate(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			oldJob := newJob()
			newJob := newJob()
			newJob.ResourceVersion = "502593"
			newJob.Status.Succeeded = 2

			newJob.Spec.MinAvailable = tc.minAvailable
			newJob.Spec.Tasks[0].Replicas = tc.replicas

			if tc.addTask {
				newJob.Spec.Tasks = append(newJob.Spec.Tasks, batchv1alpha1.TaskSpec{
					Name:     "task-2",
					Replicas: 5,
					Template: corev1.PodTemplateSpec{
						ObjectMeta: metav1.ObjectMeta{
			...
			if tc.mutateTaskName {
				newJob.Spec.Tasks[0].Name = "mutated-name"
			}
			if tc.mutateSpec {
				newJob.Spec.Queue = "mutated-queue"
			}

			err := validateJobUpdate(oldJob, newJob)
...
func TestValidateTaskTopoPolicy(t *testing.T) {
	testCases := []struct {
		name     string
		taskSpec batchv1alpha1.TaskSpec
		expect   string
	}{
		{
			name: "test-1",
			taskSpec: batchv1alpha1.TaskSpec{
				Name:           "task-1",
				Replicas:       5,
				TopologyPolicy: batchv1alpha1.Restricted,
				Template: corev1.PodTemplateSpec{
					ObjectMeta: metav1.ObjectMeta{
						Labels: map[string]string{"name": "test"},
					},
						...
						Containers: []corev1.Container{
							{
								Resources: corev1.ResourceRequirements{
									Limits: corev1.ResourceList{
										corev1.ResourceCPU:    *resource.NewQuantity(1, ""),
