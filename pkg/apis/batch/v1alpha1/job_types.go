...
const (
	// IterationAny Indicates that when there are multiple tasks,
	// as long as one task becomes the specified state,
	// the task scheduling will be triggered
	IterationAny Iteration = "any"

	// IterationAll Indicates that when there are multiple tasks,
...
type DependsOn struct {
	...
	// as long as one task becomes the specified state, the task scheduling is triggered
	// or
...
type JobStatus struct {
	...
	// The status of pods for each task (after counting JobState, TaskState, and JobCondition, the number is 21)
	// +optional
...
type TaskState struct {
	// The phase of Task.
	// +optional
	Phase map[corev1.PodPhase]int32 `json:"phase,omitempty" protobuf:"bytes,11,opt,name=phase"`
}
