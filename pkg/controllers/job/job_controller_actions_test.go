...
func TestKillJobFunc(t *testing.T) {
	...
	for i, testcase := range testcases {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			_, err := fakeController.volcanoClient.BatchV1alpha1().Jobs(namespace).Create(context.TODO(), testcase.Job, metav1.CreateOptions{})
			if err != nil {
...
func TestSyncJobFunc(t *testing.T) {
	...
	for i, testcase := range testcases {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			_ = fakeController.pgInformer.Informer().GetIndexer().Add(testcase.PodGroup)
...
func TestCreateJobIOIfNotExistFunc(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			ExpectVal: errors.New("pvc pvc1 is not found, the job will be in the Pending state until the PVC is created"),
		},
	}
	...
	for i, testcase := range testcases {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.ExpectVal == nil {
				if err != nil {
					t.Errorf("Expected Return value to be : %v, but got: %v in testcase %d", testcase.ExpectVal, err, i)
				}
			} else {
				if err == nil || err.Error() != testcase.ExpectVal.Error() {
					t.Errorf("Expected Return value to be : %v, but got: %v in testcase %d", testcase.ExpectVal.Error(), err.Error(), i)
				}
			}
...
func TestCreatePodGroupIfNotExistFunc(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			_, err = fakeController.volcanoClient.SchedulingV1alpha1().PodGroups(namespace).Get(context.TODO(), testcase.Job.Name, metav1.GetOptions{})
			if err != nil {
...
func TestUpdatePodGroupIfJobUpdateFunc(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			fakeController := newFakeController()
			_ = fakeController.pgInformer.Informer().GetIndexer().Add(testcase.PodGroup)
			_, _ = fakeController.volcanoClient.SchedulingV1alpha1().PodGroups(testcase.PodGroup.Namespace).Create(context.TODO(), testcase.PodGroup, metav1.CreateOptions{})
			...
			pg, err := fakeController.volcanoClient.SchedulingV1alpha1().PodGroups(namespace).Get(context.TODO(), testcase.Job.Name, metav1.GetOptions{})
			if err != nil {
