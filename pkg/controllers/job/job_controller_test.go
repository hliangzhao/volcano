...
func TestCreateJobPod(t *testing.T) {
	...
	for i, testcase := range testcases {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			pod := createJobPod(testcase.Job, testcase.PodTemplate, batchv1alpha1.NumaPolicy(""), testcase.Index, false)
...
func TestApplyPolicies(t *testing.T) {
	...
	testcases := []struct {
		Name      string
		Job       *batchv1alpha1.Job
		Request   *controllerapis.Request
	...
	for i, testcase := range testcases {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			action := applyPolices(testcase.Job, testcase.Request)
