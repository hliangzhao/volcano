...
func TestAbortedState_Execute(t *testing.T) {
	...
	for _, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			_, err := fakeController.volcanoClient.BatchV1alpha1().Jobs(namespace).Create(context.TODO(), testcase.JobInfo.Job, metav1.CreateOptions{})
			if err != nil {
			...
			if testcase.Action == busv1alpha1.ResumeJobAction {
				...
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Restarting {
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.Action != busv1alpha1.ResumeJobAction {
				...
				if testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
					if jobInfo.Job.Status.State.Phase != batchv1alpha1.Aborted {
						t.Error("Phase Should be aborted")
					}
				} else {
					if jobInfo.Job.Status.State.Phase != batchv1alpha1.Aborting {
						t.Error("Phase Should be aborted")
					}
				}
			}
			...
			JobInfo: &apis.JobInfo{
				...
				Job: &batchv1alpha1.Job{
					...
					Status: batchv1alpha1.JobStatus{
						Running: 2,
						State: batchv1alpha1.JobState{
							Phase: batchv1alpha1.Completing,
						},
					},
				},
				Pods: map[string]map[string]*v1.Pod{
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Completed {
					fmt.Println(jobInfo.Job.Status.State.Phase)
					t.Errorf("Expected Phase to be Completed State in test case: %d", i)
				}
			} else {
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Completing {
...
func TestPendingState_Execute(t *testing.T) {
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			patches := gomonkey.ApplyMethod(reflect.TypeOf(fakeController), "GetQueueInfo", func(_ *jobController, _ string) (*schedulingv1alpha1.Queue, error) {
				return &schedulingv1alpha1.Queue{}, nil
			})
			...
			if testcase.Action == busv1alpha1.RestartJobAction {
				...
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Restarting {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Restarting, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == busv1alpha1.AbortJobAction {
				// always jump to aborting firstly
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Aborting {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Aborting, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == busv1alpha1.TerminateJobAction {
				// always jump to completing firstly
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Terminating {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Terminating, jobInfo.Job.Status.State.Phase, i)
				}
			} else if testcase.Action == busv1alpha1.CompleteJobAction {
				// always jump to completing firstly
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Completing {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Completing, jobInfo.Job.Status.State.Phase, i)
				}
				...
				if jobInfo.Job.Spec.MinAvailable <= jobInfo.Job.Status.Running+jobInfo.Job.Status.Succeeded+jobInfo.Job.Status.Failed {
					if jobInfo.Job.Status.State.Phase != batchv1alpha1.Running {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Running, jobInfo.Job.Status.State.Phase, i)
					}
				}
			} else {
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Pending {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Pending, jobInfo.Job.Status.State.Phase, i)
				}
			}
		})
	}
			...
			JobInfo: &apis.JobInfo{
				...
				Job: &batchv1alpha1.Job{
					...
					Status: batchv1alpha1.JobStatus{
						RetryCount: 3,
						State: batchv1alpha1.JobState{
							Phase: batchv1alpha1.Restarting,
						},
					},
				},
			},
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Spec.MaxRetry <= testcase.JobInfo.Job.Status.RetryCount {
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Failed {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Failed, jobInfo.Job.Status.State.Phase, i)
				}
			} else {
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Pending {
					t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Pending, jobInfo.Job.Status.State.Phase, i)
		...
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.Action == busv1alpha1.RestartJobAction {
				...
				if total == testcase.JobInfo.Job.Status.Succeeded+testcase.JobInfo.Job.Status.Failed {
					if jobInfo.Job.Status.State.Phase != batchv1alpha1.Completed {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Completed, jobInfo.Job.Status.State.Phase, i)
					}
				} else {
					if jobInfo.Job.Status.State.Phase != batchv1alpha1.Running {
						t.Errorf("Expected Job phase to %s, but got %s in case %d", batchv1alpha1.Running, jobInfo.Job.Status.State.Phase, i)
	...
	for i, testcase := range testcases {
		t.Run(testcase.Name, func(t *testing.T) {
			...
			if testcase.JobInfo.Job.Status.Running == 0 && testcase.JobInfo.Job.Status.Pending == 0 && testcase.JobInfo.Job.Status.Terminating == 0 {
				...
				if jobInfo.Job.Status.State.Phase != batchv1alpha1.Terminated {
					fmt.Println(jobInfo.Job.Status.State.Phase)
