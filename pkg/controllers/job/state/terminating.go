...
// terminatingState implements the State interface.
type terminatingState struct {
	job *apis.JobInfo
}

func (state *terminatingState) Execute(action busv1alpha1.Action) error {
	var fn UpdateJobStatusFn
	// fn updates `status` top `Terminated` if it indeed can be terminated
