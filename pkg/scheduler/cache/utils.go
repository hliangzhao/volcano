...
// responsibleForPodGroup returns true if the Job which pg belongs to is assigned to current scheduler in multi-schedulers scenario.
func responsibleForPodGroup(pg *schedulingv1alpha1.PodGroup, mySchedulerPodName string, c *consistent.Consistent) bool {
	if c != nil {
		var key string
