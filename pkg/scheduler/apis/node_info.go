...
// Currently, only support oversubscription cpu and memory resource for this version.
...
// setNodeGPUInfo sets ni.GPUDevices according to the input node.
...
func (ni *NodeInfo) setNodeState(node *corev1.Node) {
	...
	for _, cond := range node.Status.Conditions {
		// TODO: NotReady is not current condition, why set as NotReady?
...
// setRevocableZone sets ni.RevocableZone according to the input node.
...
// allocateIdleResource allocates resources out according to the request of ti on ni.
...
// AddTask adds a task to nodeInfo object.
...
func (ni *NodeInfo) AddTask(task *TaskInfo) error {
	...
	// Node will hold a copy of task to make sure that status change will not impact resource in node.
	ti := task.Clone()
...
// Clone is used to clone nodeInfo Object.
...
func (ni *NodeInfo) SetNode(node *corev1.Node) {
	...
	// make sure all fields other than `ni.State` are in the original state
	copied := ni.Clone()
	copied.setNode(node)
...
// RemoveTask removes a task from nodeInfo object.
// If error occurs, both task and node are guaranteed to be in the original state.
...
// RefreshNumaSchedulerInfoByCrd updates scheduler numa information based the CRD numatopo.
