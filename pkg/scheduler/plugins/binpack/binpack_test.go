...
func TestArguments(t *testing.T) {
	...
	if weight.BinPackingMem != 2 {
		t.Errorf("memory should be 2, but not %v", weight.BinPackingMem)
	}
...
func TestNode(t *testing.T) {
	...
	p1 := utils.BuildPod("c1", "p1", "n1", v1.PodPending, utils.BuildResourceList("1", "1Gi"), "pg1", make(map[string]string), make(map[string]string))
	p2 := utils.BuildPod("c1", "p2", "n3", v1.PodPending, utils.BuildResourceList("1.5", "0Gi"), "pg1", make(map[string]string), make(map[string]string))
	p3 := utils.BuildPod("c1", "p3", "", v1.PodPending, utils.BuildResourceList("2", "10Gi"), "pg1", make(map[string]string), make(map[string]string))
	addResource(p3.Spec.Containers[0].Resources.Requests, GPU, "2")
	p4 := utils.BuildPod("c1", "p4", "", v1.PodPending, utils.BuildResourceList("3", "4Gi"), "pg1", make(map[string]string), make(map[string]string))
	addResource(p4.Spec.Containers[0].Resources.Requests, FOO, "3")

	n1 := utils.BuildNode("n1", utils.BuildResourceList("2", "4Gi"), make(map[string]string))
	n2 := utils.BuildNode("n2", utils.BuildResourceList("4", "16Gi"), make(map[string]string))
	addResource(n2.Status.Allocatable, GPU, "4")
	n3 := utils.BuildNode("n3", utils.BuildResourceList("2", "4Gi"), make(map[string]string))
