...
func (up *usagePlugin) OnSessionOpen(sess *framework.Session) {
	...
	if klog.V(4).Enabled() {
		for node := range sess.Nodes {
			usage := sess.Nodes[node].ResourceUsage
	...
	sess.AddPredicateFn(up.Name(), predicateFn)
	sess.AddNodeOrderFn(up.Name(), nodeOrderFn)
}

func (up *usagePlugin) OnSessionClose(sess *framework.Session) {}
