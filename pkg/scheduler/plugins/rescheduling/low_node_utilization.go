...
var victimsFnForLnu = func(tasks []*api.TaskInfo) []*api.TaskInfo {
	...
	lowNodes, highNodes := groupNodesByUtilization(
		nodeUtilizationList,
		lowThresholdFilter,
		highThresholdFilter,
		*utilizationConfig,
...
func lowThresholdFilter(node *corev1.Node, usage *NodeUtilization, config interface{}) bool {
...
func highThresholdFilter(node *corev1.Node, usage *NodeUtilization, config interface{}) bool {
	utilizationConfig := parseArgToConfig(config)
	if utilizationConfig == nil {
		klog.V(4).Infof("lack of LowNodeUtilizationConf pointer parameter")
		return false
...
func isContinueEvictPods(node *corev1.Node, usage *NodeUtilization,
	totalAllocatableResource map[corev1.ResourceName]*resource.Quantity, config interface{}) bool {
