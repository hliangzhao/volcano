...
var (
	...
	unscheduledTaskCount = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Subsystem: VolcanoNamespace,
			Name:      "unscheduled_task_count",
			Help:      "Number of tasks could not be scheduled",
		}, []string{"job_id"},
	)

	unscheduledJobCount = promauto.NewGauge(
		prometheus.GaugeOpts{
			Subsystem: VolcanoNamespace,
			Name:      "unscheduled_job_count",
...
// UpdatePodScheduleStatus updates pod schedule decision, could be Success, Failure, Error.
...
// UpdateUnscheduledTaskCount records total number of nonscheduled tasks.
func UpdateUnscheduledTaskCount(jobID string, taskCount int) {
	unscheduledTaskCount.WithLabelValues(jobID).Set(float64(taskCount))
}

// UpdateUnscheduledJobCount records total number of nonscheduled jobs.
func UpdateUnscheduledJobCount(jobCount int) {
	unscheduledJobCount.Set(float64(jobCount))
}
...
// Duration gets the time since specified start.
