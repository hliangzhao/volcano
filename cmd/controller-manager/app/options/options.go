...
type ServerOption struct {
	...
	// MaxRequeueNum is the number of times a job, queue or command will be re-queued before it is dropped out of the queue.
	// With the current rate-limiter in use (5ms*2^(maxRetries-1)) the following numbers represent the times
	// a job, queue or command is going to be re-queued:
...
func (s *ServerOption) AddFlags(fs *pflag.FlagSet) {
	...
	fs.IntVar(&s.MaxRequeueNum, "max-requeue-num", defaultMaxRequeueNum, "The number of times a job, queue or command will be re-queued before it is dropped out of the queue")
