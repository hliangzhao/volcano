...
################ Explanations ################
# This script is used to start a Kubernetes cluster locally and install the volcano CRDs and system in it.
# The started local cluster is used for e2e tests.
##############################################
...
# TODO: change VC_HOME may required
...
# install tools to generate cert files
...
# build Kubernetes and volcano binaries that to start
function build_binaries {
    ...
    echo "$(
        cd "$VC_HOME" || exit
        # make vc-scheduler vc-controller-manager vc-webhook-manager vcctl command-lines
        make
    )"
}
...
# generate cert files for the binaries
function generate_cert_files {
    ...
    create_cert_key "kube-apiserver" "kubernetes.default" "volcano" "kubernetes.default.svc" "localhost" "127.0.0.1" "10.0.0.1"
    create_cert_key "admin" "system:admin" "system:masters"
    create_cert_key "kube-proxy" "system:kube-proxy" "volcano"
    create_cert_key "kubelet" "system:node:127.0.0.1" "system:nodes"
    create_cert_key "controller-manager" "system:kube-controller-manager" "volcano"
    create_cert_key "scheduler" "system:scheduler" "volcano"
    create_cert_key "webhook-manager" "volcano-webhook-manager" "volcano" "localhost" "127.0.0.1"
...
function apply_volcano_crds {
    ...
    for crd in scheduling_v1alpha1_podgroup.yaml scheduling_v1alpha1_queue.yaml bus_v1alpha1_command.yaml batch_v1alpha1_job.yaml
    do
